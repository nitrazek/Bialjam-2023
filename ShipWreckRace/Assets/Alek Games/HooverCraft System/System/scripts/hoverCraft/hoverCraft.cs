    ...
    public class hoverCraft : MonoBehaviour
    {
        ...
        private void gatherInput()
        {
            ...
            if (inputType == InputT.axes)
            {
                ...
                if(playerId == 1)
                {
                    x += Input.GetKey(GameConfig.p1_right) ? 1 : 0;
                    x -= Input.GetKey(GameConfig.p1_left) ? 1 : 0;
                    y += Input.GetKey(GameConfig.p1_forward) ? 1 : 0;
                    y -= Input.GetKey(GameConfig.p1_back) ? 1 : 0;
                }
                else if(playerId == 2)
                {
                    x += Input.GetKey(GameConfig.p2_right) ? 1 : 0;
                    x -= Input.GetKey(GameConfig.p2_left) ? 1 : 0;
                    y += Input.GetKey(GameConfig.p2_forward) ? 1 : 0;
                    y -= Input.GetKey(GameConfig.p2_back) ? 1 : 0;
                }
